...
### Added
- 🪪 **所有交易所的战绩上报都带账户身份**：此前只有 LTP 能说清「这笔成交来自哪个账号」，其它交易所只能靠「你在这个所只绑了一把 Key」来匹配，绑第二个账号就失效。现在 Binance / OKX 用交易所自身 UID、Hyperliquid / Aster 用钱包地址生成同样的非敏感 `account_ref`；Gate / Bitget 的接口拿不到身份，保持原有单 Key 匹配。每个账号只查一次并缓存，上报周期不增加任何请求。

### Changed
- 🏦 **策略暂时只支持 Binance、OKX、LTP**：这三个是所有实盘实例正在跑、且手续费模型、订单语义、错误码都对着真实响应逐条核验过的；其它交易所在创建策略时会被挡下并说明原因。
- 🗒️ 策略说明文案不再出现具体币种名称，避免被读成推荐。

### Removed
- 📡 **移除「跨所价差带」策略**：它是唯一一个从不下单的策略(只采集两所可执行价差用于阈值分析)。已采集的历史数据保留在数据库中，未做删除。

### Fixed
- 🔁 **汇率套利平仓路径的多处卡死**(本次最重要)：平仓阶段此前从不与交易所核对真实仓位，内存数量偏大时 reduceOnly 单会被拒或少成交，平仓永远结束不了；小于交易所最小下单量的残腿会拖住整轮，现在该腿让步、另一条腿照常推进。
- 🌀 **开仓完成判定空转**：目标量低于交易所最小下单量时会陷入无限轮次 —— 实测 4000 个 tick、1997 轮不退出，还在与实盘共用的 IP 上每两秒打四次请求。完成判定改为按「相对容差」与「交易所真正接受的最小量」取大者。
- 🧯 **maker 下单被拒后残留意图**：会让之后每个 tick 直接报错，甚至只发出一条腿、让对冲策略裸露方向。maker 现在与 taker 用同一套失败分类，每条腿都会尝试。
- 🔓 **平仓卡住后阶段永远停在 closing**：导致开仓和删除都被拒绝、界面上的救援按钮也不显示。现在交易所确认两腿都已清空时会自动释放。
- 🛟 引擎 tick 的异常处理本身可能把引擎杀死(数据库抖动时)，现在处理器只记录不再向上抛，循环始终存活。
- 📌 DMan / 针刺网格：LTP 异步撤单窗口期内的 maker 平仓意图不再丢失。
